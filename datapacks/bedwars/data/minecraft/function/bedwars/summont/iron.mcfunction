...
execute store result score temp board run data get entity @e[type=item,distance=..2,limit=1,sort=nearest,nbt={Item:{id:"minecraft:iron_ingot"}}] Item.count
execute at @s as @a[tag=bw.play,gamemode=!spectator,distance=0..2] run scoreboard players add temp1 board 1
execute if score temp1 board matches ..1 run function fix:bw/irspan
