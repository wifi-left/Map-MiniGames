...
execute if score @s bw.team matches 4 run tellraw @s ["\u00a7b您已经加入过这个队伍了。"]
execute if score @s bw.team matches 4 run return 0

function minecraft:bedwars/before/count_sel_team_players

execute if score bw.green.1 tick >= bw.wait.perteammax tick run tellraw @s ["\u00a7c您无法加入此队伍：队伍已满。"]
execute if score bw.green.1 tick >= bw.wait.perteammax tick run return 0

scoreboard players set @s bw.team 4
tellraw @s ["\u00a7b您加入了\u00a7a绿队"]
execute if score bw.state state matches -3..0 run title @s actionbar ["\u00a7b\u00a7l您选择的队伍：\u00a7a绿队"]
