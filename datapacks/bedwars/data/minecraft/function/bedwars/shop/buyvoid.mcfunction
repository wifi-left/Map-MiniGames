...
execute if score @s bw.tmp.gd matches 8.. run tellraw @s ["§a你购买了§6虚空回城卷轴(一次性) * 1"]
execute if score @s bw.tmp.gd matches 8.. run give @s paper[can_place_on=[{blocks:"#minecraft:bwplace"}],tooltip_display={hidden_components:[can_place_on,can_break]},can_break=[{blocks:"#minecraft:bedblocks"}],item_name='卷轴',custom_name={text:"虚空回城卷轴(一次性)",color:green,italic:false},lore=[{text:"放入背包即可在落入虚空时返回出生点",color:aqua,italic:true}],custom_data={type:"bw:void"}]
