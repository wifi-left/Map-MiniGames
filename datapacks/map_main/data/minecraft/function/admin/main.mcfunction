...
tellraw @s ["§e- 大厅音乐数据包 ",{"text":"[启用(默认)]","color":"green","click_event":{"action":"run_command","command":"/datapack enable \"file/LobbyMusicPack_v4.zip\""}},{"text":"[禁用]","color":"red","click_event":{"action":"run_command","command":"/datapack disable \"file/LobbyMusicPack_v4.zip\""}}]
tellraw @s [""]
