...
tellraw @s ["§e- 派对模式（加入游戏自动加入小游戏派对） ",{"text":"[启用]","color":"green","click_event":{"action":"run_command","command":"/function admin/partymode/on"}},{"text":"[禁用]","color":"aqua","click_event":{"action":"run_command","command":"/function admin/partymode/off"}}]
tellraw @s ["§e- 返回大厅“一言” ",{"text":"[启用(默认)]","color":"green","click_event":{"action":"run_command","command":"/data modify storage statics:settings hub_msg.oneword.allow set value true"}},{"text":"[禁用]","color":"aqua","click_event":{"action":"run_command","command":"/data modify storage statics:settings hub_msg.oneword.allow set value false"}}]
