...
execute as @s[tag=wolf.killed] if score wolf.round board matches 2.. run tellraw @s ["\u00a7c你被杀了。今夜不是第一晚，\u00a7l你无法对自己使用解药。"]

execute as @s[tag=!wolf.killed] if score wolf.witch.rescue board matches 1.. run item replace entity @s hotbar.2 with warped_fungus_on_a_stick{display:{Name:'"\\u00a7a使用解药 - \\u00a77右键点击"'},wolfItem:4,CustomModelData:8}
