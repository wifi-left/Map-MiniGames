...
execute if score @s LRS_JOB matches 8 unless entity @a[tag=wolf.knight,team=wolfpeople,gamemode=adventure] run item replace entity @s hotbar.7 with warped_fungus_on_a_stick{display:{Name:'"\\u00a7e骑士之刃 - \\u00a77右键发动技能"'},wolfItem:7,CustomModelData:4}
# wolf.setting.knight - 1: 只有一次
execute if score @s LRS_JOB matches 8 if entity @a[tag=wolf.knight,team=wolfpeople,gamemode=adventure] unless score wolf.setting.knight state matches 1.. run item replace entity @s hotbar.7 with warped_fungus_on_a_stick{display:{Name:'"\\u00a7e骑士之刃 - \\u00a77右键发动技能"'},wolfItem:7,CustomModelData:4}
