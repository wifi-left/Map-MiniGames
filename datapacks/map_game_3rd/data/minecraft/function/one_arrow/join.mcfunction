...
tellraw @a [{"selector":"@s"}," \u00a7a加入了 \u00a7b一箭毙命"]
title @s title ["\u00a7b一箭毙命"]
title @s subtitle ["小心老六！"]
tellraw @s ["\n \u00a7b\u00a7l一箭毙命\u00a76：活到最后！所有箭矢的攻击都是一箭毙命的。\n"]
execute in overworld run tp @s -79 37 200 0 0
xp set @s 0 levels
xp set @s 0 points
execute at @s run playsound entity.player.levelup player @s ~ ~ ~ 10 1 1
clear @s
effect clear @s
execute if score one_arrow.state state matches 1.. run function minecraft:one_arrow/spec
