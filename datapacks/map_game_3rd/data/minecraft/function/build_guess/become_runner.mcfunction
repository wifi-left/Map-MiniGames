...
title @s title ["\u00a7b本次词汇长度：",{nbt:"build_guess.length",storage:"minecraft:temp",color:yellow,interpret:false,plain:true}]
title @s subtitle ["\u00a76请使用物品栏中书与笔进行书写"]
tellraw @s ["\u00a76请使用物品栏中书与笔进行书写（无需署名）"]
tellraw @s ["\u00a7b本次词汇长度：",{nbt:"build_guess.length",storage:"minecraft:temp",color:yellow,interpret:false,plain:true}]
