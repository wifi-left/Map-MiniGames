...
execute unless score build_guess.state state matches 1.. if score wait.player tick matches ..1 run tellraw @s ["§c人数不够！请等待至少2人！"]
execute unless score build_guess.state state matches 1.. if score wait.player tick matches ..1 run playsound entity.enderman.teleport player @s ~ ~ ~ 1 0 1
execute unless score build_guess.state state matches 1.. if score wait.player tick matches 2.. run function minecraft:build_guess/start
