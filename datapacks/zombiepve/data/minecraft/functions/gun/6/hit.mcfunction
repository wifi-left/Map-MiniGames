tag @s add sel
execute unless data entity @s {Invulnerable:1b} run damage @s 5 minecraft:player_gun by @a[tag=hit.player,limit=1]
...
tellraw @a[tag=hit.player] ["\u00a76+50\u00a76 Coins"]
...
execute as @a[tag=hit.player] at @s run scoreboard players add @s zombie.coin 50
tag @s remove sel
# say 1