...
execute as @a[team=Who_Killer_Text,gamemode=adventure] at @s if score @s LRS_UUID = @a[limit=1,scores={SP=-1..,LRS_JOB=4},gamemode=adventure] SP run tellraw @a[team=Who_Killer_Text,tag=GOABLE.SPEC] ["\u00a76 --> ",{"selector":"@a[scores={SP=-1..,LRS_JOB=4},gamemode=adventure]"},"\u00a7a保护了 ",{"selector":"@a[tag=lrs.sel]"}]
