...
execute if entity @a[limit=1,scores={SP=-1,LRS_JOB=2},gamemode=adventure] if score vitch.help board matches 1.. run tellraw @a[team=Who_Killer_Text,tag=GOABLE_SPEC] ["\u00a76 --> ",{"selector":"@a[scores={SP=-1,LRS_JOB=2},gamemode=adventure]"},"\u00a7a救助了 ",{"selector":"@a[tag=lrs.out]"}]
