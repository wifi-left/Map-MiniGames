...
execute unless entity @s[tag=killer.saver] at @e[tag=killer.temp] run summon minecraft:armor_stand ~ ~ ~ {CustomNameVisible:0b,NoAI:1b,Silent:1b,NoGravity:1b,Glowing:0b,Tags:["killer.bow","new"],Invisible:true,ShowArms:true,NoBasePlate:true,DisabledSlots:2039326}

execute if entity @s[tag=killer.saver] at @e[tag=killer.temp] run summon minecraft:armor_stand ~ ~ ~ {CustomName:["\u00a7a弓\u00a77 - 右键获取"],CustomNameVisible:1b,NoAI:1b,Silent:1b,NoGravity:1b,Glowing:1b,Tags:["killer.bow","new"],equipment:{mainhand:{id:"minecraft:bow",count:1,components:{"minecraft:custom_data":{dtb:1b},"minecraft:unbreakable":{},"minecraft:custom_name":["\u00a7a弓"],"minecraft:enchantments":{"minecraft:power":10s}}}},Invisible:true,ShowArms:true,NoBasePlate:true,DisabledSlots:2039326}
