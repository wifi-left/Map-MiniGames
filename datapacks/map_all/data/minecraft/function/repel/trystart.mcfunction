...
execute as @a[team=wait.repel,gamemode=adventure] run scoreboard players add wait.player tick 1
...
execute unless score wait.player tick matches 2.. run tellraw @s ["§c人数不够！请等待至少2人！"]
execute unless score wait.player tick matches 2.. run playsound entity.enderman.teleport player @s ~ ~ ~ 1 0 1
execute if score wait.player tick matches 2.. run function minecraft:repel/start
