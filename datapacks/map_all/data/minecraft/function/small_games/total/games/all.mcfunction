...
data modify storage minecraft:temp random_games_all.games append value {id:33,name:"\u00a74挖矿达人",prefix:"[街机游戏]",desc:"要致富，先挖矿！",state_var:"mining.state state",start_func:["execute as @a[tag=play.total] run function mining/join","function mining/trystart"]}
data modify storage minecraft:temp random_games_all.games append value {id:2,name:"\u00a7bTNTRUN",prefix:"[策略游戏]",desc:"快跑！你脚下的方块会自己掉落。",state_var:"snow.state state",start_func:["execute as @a[tag=play.total] run function snow/join","scoreboard players set snow.tntrun state 1","function snow/trystart"]}
data modify storage minecraft:temp random_games_all.games append value {id:3,name:"\u00a76掘一死战",prefix:"[策略游戏]",desc:"挖掉你对手脚下的方块！",state_var:"snow.state state",start_func:["scoreboard players set snow.tntrun state 0","execute as @a[tag=play.total] run function snow/join","function snow/trystart"]}
data modify storage minecraft:temp random_games_all.games append value {id:30,name:"\u00a7c掘一死战(爆炸弓版)",prefix:"[策略游戏]",desc:"用弓射出箭把对手的脚下方块破坏！",state_var:"snow.state state",start_func:["scoreboard players set snow.tntrun state 3","execute as @a[tag=play.total] run function snow/join","function snow/trystart"]}
...
data modify storage minecraft:temp random_games_all.games append value {id:9,name:"\u00a76烫手的山芋",prefix:"[策略游戏]",desc:"进入对方守护的得分区，夺得胜利！",state_var:"hotpot.state state",start_func:["execute as @a[tag=play.total] run function hotpotever/join","function hotpotever/teststart"]}
...
data modify storage minecraft:temp random_games_all.games append value {id:21,name:"\u00a78密室杀手",prefix:"[废案]",desc:"此游戏为废案，游玩体验可能不好，不建议游玩。",state_var:"killer.state state",start_func:["execute as @a[tag=play.total] run function killerever/join","function killerever/teststart"]}
